# -*- coding: utf-8 -*-
"""
Created on Tue Aug 29 09:44:28 2023
...
from generate_data_NonPoly import DataSet
import time
import utils
import scipy.io
...
class Model(object):
    ...
    @utils.timing
    def compile(self, basis_order, basis_xi_order, gauss_variance, type, diffusion_term, xi_term, gauss_samp_way, lhs_ratio):
    ...
    def train(self, gauss_samp_number, lam, STRidge_threshold):
        ...
        print("------------------self.b.detach().numpy().shape", self.b.detach().numpy().shape)
        ...
        L2_error = torch.norm(-2*x * torch.exp(-x**2) - self.drift_est(x))
...
if __name__ == '__main__':
    ...
    dataset = DataSet(t, dt=0.0001, samples_num=samples, dim=1,\
                      diffusion_term=diffusion, xi_term=xi, alpha_levy = 3/2, initialization=torch.normal(0, 0.2,[samples, 1]),
    ...
    model.compile(basis_order=4, basis_xi_order=1, gauss_variance=0.85, type='LMM_2_nonequal', diffusion_term=diffusion, xi_term=xi,
