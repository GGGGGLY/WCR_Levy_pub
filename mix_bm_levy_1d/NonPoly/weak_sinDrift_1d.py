...
class Model(object):
    ...
    def build_basis(self): # \Lambda matrix
        ...
        self.basis1_number = int(np.math.factorial(self.dimension+self.basis_order)
                /(np.math.factorial(self.dimension)*np.math.factorial(self.basis_order))) + 2 
        #+2 means adding {sinx, cosx}      
        ...
        for it in range(self.t_number):
            ...
            Theta[:,0] = 1
            basis_count1 += 1
            for i in range(self.basis1_number -2):
                Theta[:, i] = X[:, 0]**i
            Theta[:, -2] = torch.sin(X[:, 0])
            Theta[:, -1] = torch.cos(X[:, 0])
            basis1.append(Theta)
            # print("X", X)
        ...
        """
        basis_theta = {1, x, x^2, x^3, sinx, cosx}
        ...
        self.basis_number = 2*self.basis1_number + self.basis2_number
    ...
    def train(self, sample_num, gauss_samp_number, lam, STRidge_threshold):
        ...
        for i in range(1, self.basis1_number-2):
            drift.extend([" + ", self.zeta[i].numpy(), 'x^', i])
        drift.extend([" + ", self.zeta[self.basis1_number-2].numpy(), 'sinx'])
        drift.extend([" + ", self.zeta[self.basis1_number-1].numpy(), 'cosx'])
...
if __name__ == '__main__':
    ...
    model.compile(basis_order=1, gauss_variance=0.45, type='LMM_2_nonequal', xi_term=xi,\
                  gauss_samp_way='lhs', lhs_ratio=0.8)
    model.train(sample_num = samples, gauss_samp_number=150, lam=0.0, STRidge_threshold=0.05)
