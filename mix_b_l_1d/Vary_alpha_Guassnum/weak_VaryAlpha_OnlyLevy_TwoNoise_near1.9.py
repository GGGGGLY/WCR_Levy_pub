# -*- coding: utf-8 -*-
"""
Created on Wed Jul 19 21:38:52 2023
    ...
    def computeAb(self, gauss):
        ...
        for ld in range(self.dimension):
            for kd in range(self.dimension):
                ...
                A[:, H_number+1] = E #####sp func for d-dim? or 1-d?
...
if __name__ == '__main__':
    ...
    MRE_matrix = np.zeros((4,5))
    times = 0
    for gauss_var in np.linspace(0.7, 1.0, 4):
        MRE_list = []
        for alpha in np.linspace(1.5, 1.99, 10):
            np.random.seed(6)
            torch.manual_seed(6)
            samples = 10000
            ...
            model.compile(basis_order=3, gauss_variance=gauss_var, type='LMM_2_nonequal', drift_term=drift, diffusion_term=diffusion, xi_term=xi,
                          gauss_samp_way='lhs', lhs_ratio=1.0)
            MRE = model.train(gauss_samp_number=100, lam=0.0, STRidge_threshold=0.3)
            MRE_list.append(MRE) 
            
        MRE_matrix[[times],:] = np.asarray(MRE_list)
        times = times+1
        
    min_mre = MRE_matrix.min(0) #每行最小值
    MRE_min = min_mre
            
    xx = np.linspace(1.5, 1.99, 10)
    yy1 = np.array(MRE_min, dtype = float)
    plt.plot(xx, yy1, linewidth=2.0) 
    #plt.xlabel('Alpha', fontsize=12)
    #plt.ylabel('MRE', fontsize=12)
    #plt.tick_params(labelsize=12)
    #plt.title('MREs Varying Alpha: Gauss number =100', fontsize=13)
    #plt.show()
    ...
    for gauss_var in np.linspace(0.7, 1.0, 4):
        MRE_list = []
        for alpha in np.linspace(1.85, 1.97, 5):
            samples = 15000
            dataset = DataSet(t, dt=0.0001, samples_num=samples, dim=1,
                              drift_term=drift, diffusion_term=diffusion, xi_term=xi, alpha_levy = alpha, initialization=torch.normal(0, 0.2,[samples, 1]),\
                              explosion_prevention=False) #initialization=torch.randint(-1000,1000,[10000, 1])
    ...
    xx = np.linspace(1.85, 1.97, 5)
    yy2 = np.array(MRE_min, dtype = float)
    plt.plot(xx, yy2, linewidth=2.0) 
    #plt.xlabel('Alpha', fontsize=12)
    #plt.ylabel('MRE', fontsize=12)
    #plt.tick_params(labelsize=12)
    #plt.title('MREs Varying Alpha: Gauss number =100', fontsize=13)
    ...
    xx = np.linspace(1.85, 1.97, 5)
    yy3 = np.array(MRE_min, dtype = float)
    plt.plot(xx, yy3,".-", linewidth=2.0) 
    ...
    plt.tick_params(labelsize=12)
    plt.title('MREs Varying Alpha: Gauss number =100', fontsize=13)
    #plt.legend(['α = 1.5', 'α = 1.6', 'α = 1.7', 'α = 1.8', 'α = 1.9'])
    plt.legend(['N=10000', 'N=15000', 'N=20000'], fontsize=13) #loc='upper left'
    plt.show()
