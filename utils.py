...
class SinTest(torch.nn.Module):
    def __init__(self, mu, sigma, device):
        super(Gaussian, self).__init__()
        '''
        mu is the center of sin function
        sigma is the frequency of sin function

        f = sin(omega*(x- mu))
        width is -0.5pi,0.5pi

        '''
        self.mu = mu.to(device)
        self.sigma = sigma.to(device)
        self.dim = mu.shape[0]
        self.width = 0.5*np.pi
        self.device = device

    def gaussZero(self, x):
        func = 1
        for d in range(self.dim):
            func = func * torch.sin(self.sigma*(x[:, :, d] - self.mu[d]))
        return func

    def gaussFirst(self, x):
        func = torch.ones([x.shape[0], x.shape[1], x.shape[2]]).to(self.device)
        for k in range(self.dim):
            for d in range(self.dim):
                if d == k:
                    func[:, :, k] = func[:, :, k] *self.sigma* torch.cos(self.sigma*(x[:, :, d] - self.mu[d]))
                else:
                    func[:, :, k] = func[:, :, k] *torch.sin(self.sigma*(x[:, :, d] - self.mu[d]))
        return func

    def gaussSecond(self, x):
        func = torch.ones([x.shape[0], x.shape[1], x.shape[2], x.shape[2]]).to(self.device)
        for k in range(x.shape[2]):
            for j in range(x.shape[2]):
                if k == j:
                    for d in range(self.dim):
                        func[:, :, k, j] = func[:, :, k, j]*torch.sin(self.sigma*(x[:, :, d] - self.mu[d]))
                    func[:, :, k, j] = -func[:, :, k, j]*self.sigma**2
                else:
                    for d in range(self.dim):
                        if d == k:
                            func[:, :, k, j] = func[:, :, k, j]*self.sigma*torch.cos(self.sigma*(x[:, :, d] - self.mu[d]))
                        elif d == j:
                            func[:, :, k, j] = func[:, :, k, j]*self.sigma*torch.cos(self.sigma*(x[:, :, d] - self.mu[d]))
                        else :
                            func[:, :, k, j] = func[:, :, k, j]*torch.sin(self.sigma*(x[:, :, d] - self.mu[d]))
