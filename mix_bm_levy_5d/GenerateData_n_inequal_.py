...
class DataSet(object):
    ...
    def levy_variable(self):
        V = (torch.rand(self.samples_num,self.dim)*np.pi - np.pi/2)*0.999
        U = torch.rand(self.samples_num,self.dim)*0.999
        W = -torch.log(U+1e-4)
...
if __name__ == '__main__':
    ...
    xi = torch.tensor([1.5, 1.0, 1.2, 1.0, 1.8], dtype=torch.float)
    samples_num = 6000 
