...
class DataSet(object):
    ...
    #def levy_variable(self):
    #    V = (torch.rand(self.samples_num,self.dim)*np.pi - np.pi/2)
    #    U = torch.rand(self.samples_num,self.dim)
    #    W = -torch.log(U)
   #     X =  torch.sin(self.alpha_levy*V)/torch.cos(V)**(1/self.alpha_levy)*(torch.cos((1-self.alpha_levy)*V)/W)**((1-self.alpha_levy)/self.alpha_levy) * np.random.normal(1,0.001)
    #    return X
...
if __name__ == '__main__':
    ...
    diffusion = torch.tensor([1.0]).repeat(dim)
    ...
    dataset = DataSet(torch.tensor([0,1.0]), dt=0.001, samples_num=samples_num, dim=dim, \
                      ...
                      alpha_levy = 3/2, initialization=torch.normal(0, 0.2,[samples_num, dim]), \
