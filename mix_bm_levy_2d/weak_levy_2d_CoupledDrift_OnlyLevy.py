# -*- coding: utf-8 -*-
"""
Created on Tue Feb 21 16:32:28 2023
...
from GenerateData_n_dependence_OnlyLevy import DataSet
...
class Gaussian(torch.nn.Module):
    ...
    def LapGauss_VaryDim(self,x, g0):
        ...
        for k in range(x.shape[2]):
            ...
            func_k = (np.sqrt(1/(np.sqrt(2)*self.sigma)) )** self.lap_alpha * sp.gamma( (1 + self.lap_alpha)/2 )* 2**self.lap_alpha / sp.gamma(1/2) * \
...
class Model(object):
    ...
    def sampleTestFunc(self, samp_number):
        ...
        if self.gauss_samp_way == 'lhs':
            lb = torch.tensor([self.data[:, :, i].min()*(1/2) for i in range(self.dimension)]).to(self.device)
            ub = torch.tensor([self.data[:, :, i].max()*(1/2) for i in range(self.dimension)]).to(self.device)
    ...
    def train(self, gauss_samp_number, Xi_type, lam, STRidge_threshold, only_13=None):
        ...
        #        = (1.7973 * (self.zeta.squeeze()[self.dimension * self.basis1_number:]))**(2/3)
        #    print("Xi term: ", "diag ", self.zeta.squeeze()[self.dimension*self.basis1_number: ].numpy())
        #else:
        #    print("Xi term: ", (1.7973 * (self.zeta.squeeze()[self.dimension * self.basis1_number: ]))**(2/3).numpy())

        if self.Xi_type == "Diag":
            self.zeta.squeeze()[self.dimension * self.basis1_number: ] \
...
if __name__ == '__main__':
    ...
    #drift = torch.tensor([0.0, 1.0, 0.0, -1.0]).repeat(dim, 1)
    ...
    alpha =3/2
    Xi_type = "Diag" 
    dataset = DataSet(t, dt=0.001, samples_num=sample, dim=dim, drift_term=drift, \
                      xi_term = xi, alpha_levy = 3/2, initialization=torch.normal(0, 0.2,[sample, dim]), \
    ...
    model.compile(basis_order=3, basis_xi_order=1, gauss_variance=0.75, type='LMM_3', \
                  drift_term=drift, xi_term = xi, drift_independence=False,  \
                  xi_independence=True, gauss_samp_way='lhs', lhs_ratio=0.65) 
    
    # For 1 dimension, "drift_independence" and "diffusion_independence"  make no difference
    model.train(gauss_samp_number=100, Xi_type = Xi_type, lam=0.0, STRidge_threshold=0.5, only_13="None")



# t = torch.linspace(0, 1, 10)
# lb = min*(1/2), ub = max*(1/2)

# smaple = 20000, gauss_samp_number=150, lam=0.0, STRidge_threshold=0.3, gauss_variance=0.65, lhs_ratio=0.75
# tensor([[ 0.0000],
#        [10.6385],
#        [ 0.8284], ############
 #       [ 0.0000],
...
#        [ 1.1804],
#        [ 0.9707]])
#Xi term:  diag  [1.1169088 0.9803536]
#Maximum relative error:  0.14782143
#Maximum index:  tensor(4)
#'train' took 37.978390 s


# smaple = 20000, gauss_samp_number=150, lam=0.0, STRidge_threshold=0.3, gauss_variance=0.65, lhs_ratio=1.0
# tensor([[ 0.0000],
#        [10.7457],
#        [-2.2336], #### 误差挺大的
#        [ 0.0000],
...
#        [ 1.1532],
#        [ 0.9667]])
#Xi term:  diag  [1.0996896 0.9776832]
#Maximum relative error:  0.1491313
#Maximum index:  tensor(4)
#'train' took 33.881769 s


# smaple = 20000, gauss_samp_number=150, lam=0.0, STRidge_threshold=0.35, gauss_variance=0.6, lhs_ratio=0.7
# 也有零项识别成非零

# smaple = 20000, gauss_samp_number=140, lam=0.0, STRidge_threshold=0.35, gauss_variance=0.7, lhs_ratio=0.7
# tensor([[ 0.0000],
#        [10.6191],
#        [ 0.0000],
#        [ 0.0000],
...
#        [ 1.1296],
#        [ 0.8932]])
#Xi term:  diag  [1.084654   0.92744315]
#Maximum relative error:  0.1488601
#Maximum index:  tensor(4)
#'train' took 32.095488 s

# smaple = 20000, gauss_samp_number=140, lam=0.0, STRidge_threshold=0.35, gauss_variance=0.72, lhs_ratio=0.65
#zeta:  torch.Size([22, 1]) tensor([[ 0.0000],
#        [10.6297],
#        [ 0.0000],
#        [ 0.0000],
#        [ 0.0000],
...
#        [ 1.1013],
 #       [ 0.8688]])
#Xi term:  diag  [1.0664201 0.9105062]
#Maximum relative error:  0.14805508
#Maximum index:  tensor(4)
#'train' took 26.775740 s

# smaple = 20000, gauss_samp_number=140, lam=0.0, STRidge_threshold=0.5, gauss_variance=0.75, lhs_ratio=0.7
# tensor([[ 0.0000],
 #       [10.7241],
#        [ 0.0000],
#        [ 0.0000],
...
#        [ 1.0369],
 #       [ 0.8559]])
#Xi term:  diag  [1.0244601 0.9014578]
#Maximum relative error:  0.14467824
#Maximum index:  tensor(5)
#'train' took 24.912160 s

# smaple = 20000, gauss_samp_number=120, lam=0.0, STRidge_threshold=0.5, gauss_variance=0.75, lhs_ratio=0.65
#tensor([[ 0.0000],
#        [10.7562],
#        [ 0.0000],
#        [ 0.0000],
...
#        [ 1.0306],
#        [ 0.8721]])
#Xi term:  diag  [1.0202801  0.91281974]
#Maximum relative error:  0.14212286
#Maximum index:  tensor(5)
#'train' took 28.672177 s

#-----------------------------------------
# smaple = 20000, gauss_samp_number=100, lam=0.0, STRidge_threshold=0.5, gauss_variance=0.75, lhs_ratio=0.65
#zeta:  torch.Size([22, 1]) tensor([[ 0.0000],
#        [10.8513],
#        [ 0.0000],
#        [ 0.0000],
...
#        [ 0.9888],
#        [ 0.8841]])
#Xi term:  diag  [0.992549  0.9211781]
#Maximum relative error:  0.13628745
#Maximum index:  tensor(5)
#'train' took 25.059234 s
#--------------------------------------
