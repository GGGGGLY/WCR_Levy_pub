...
from GenerateData_n_CoupleDrift import DataSet
...
if __name__ == '__main__':
    ...
    alpha =3/2
    Xi_type = "Diag" 
    dataset = DataSet(t, dt=0.001, samples_num=samples, dim=dim, drift_term=drift, \
    ...
    model.compile(basis_order=3, gauss_variance=0.6, type='LMM_3', drift_term=drift, \
                  xi_term = xi, drift_independence=False, xi_independence=True, gauss_samp_way='lhs', lhs_ratio=1.0) 
    
    model.train(gauss_samp_number=350,  Xi_type = Xi_type, lam=0.0, STRidge_threshold=0.5, only_13="None")



# t = torch.linspace(0, 1, 11)
# E将nan换成0了，nan几乎都是0.0000e-10这种
# smaple = 20000, gauss_samp_number=300, lam=0.0, STRidge_threshold=0.4, gauss_variance=0.65, lhs_ratio=0.6
# 不收敛
#怎么调E都几乎是全0矩阵

##用potential试一下

# smaple = 20000, gauss_samp_number=300, lam=0.0, STRidge_threshold=0.4, gauss_variance=0.65, lhs_ratio=0.8
#  误差有些大

# smaple = 20000, gauss_samp_number=200, lam=0.0, STRidge_threshold=0.4, gauss_variance=0.6, lhs_ratio=1.0
# tensor([[  0.0000], [  0.0000],  [  0.0000],  [  0.0000],  [  0.0000],  [ -1.4914], [  0.0000], [  0.0000], [ -2.3550], [  0.0000],
#        [ -5.9985], [ -1.6496],  [ -0.4795], [ -7.8694], [ -0.7322], [  2.8271], [  0.9997], [ -3.0905], [ -1.4243], [  0.0000],

#        [  0.0000], [  0.0000], [  0.0000], [  0.8319],  [  0.9899],  [  0.0000], [ -0.9122], [  0.6465], [  0.0000], [ -1.2522],
#        [  0.8026], [  0.0000],  [-14.6098], [ -4.6499], [  7.1510], [  0.0000], [ -6.1456], [  4.9666], [ -6.6120], [  0.0000],
 
#       [  0.0000], [  0.0000], [  0.0000], [  2.8869], [  0.0000],[  4.5819],[  1.1912], [  0.0000], [  0.5986], [  0.0000],
#        [  0.0000], [ 13.3254], [-15.1787], [  0.0000], [  0.0000],[  3.7072], [ -2.0682], [ -3.2764],[  0.0000],[ -5.3196], 

#[  1.1558], [  1.2612],[  0.8889]])
#Xi term:  diag  [1.1013181  1.1673379  0.92448384]
#Maximum relative error:  2.7946732
