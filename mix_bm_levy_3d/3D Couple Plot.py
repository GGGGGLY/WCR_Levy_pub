            ...
            if t0 == t1:
                ...
                for i in range(t.shape[0] - 1):
                   # dL = self.levy_rv(y, torch.tensor(self.dt))
                    ...
                    y = y + self.hat3d_estimate(y, self.drift_term)* self.dt + torch.pow(torch.tensor(self.dt), 1/self.alpha_levy) *\
    ...
    def get_data(self, plot_hist=False):
        ...
        if plot_hist:
            for i in range(self.dim):
                plt.figure()
                plt.hist(x=data[-1, :, i].numpy(), bins=80, range=[-5,5], density=True, color = 'mediumpurple')
...
if __name__ == '__main__':
    ...
    drift = torch.tensor([[0, 11.0432, 0, 0, 0, 0, 0, 0, 0, 0, -4.4727, 0, 0, -4.4839, 0, -4.2990, 0, 0, 0, 0],
                          [0, 0, 11.8475, 0, 0, 0, 0, 0, 0, 0, 0, -4.5651, 0, 0, 0, 0, -4.7658, 0, -4.9104, 0],
                         [0, 0, 0, 10.8256, 0, 0, 0, 0, 0, 0, 0, 0, -4.5086, 0, 0, 0, 0, -4.0030, 0, -4.4259]])
    
    xi = torch.tensor([1.12534, 0.98300, 1.1436028])
    ...
    xd = x=data[-1, :, 0].numpy()
    yd = x=data[-1, :, 1].numpy()
    ax1.scatter3D(xd,yd,zd, c=xd, cmap='viridis')  #绘制散点图
    plt.title('Estimated particle distribution density at the final moment',fontsize=12)
    
    #ax1.contour(xd,yd,zd,zdir='z', offset=-3,cmap="rainbow")
    plt.show()
